	...
	<head>
		...
		function update(str) {
		    ...
		    function define_routes() {
		        ...
		        Path.map("#E/params/:id/parse").before(function () {
		            var param = this.params["id"];
		        ...
		        Path.map("#E/params/:id/check").leaving(function () {
		            var param = this.params["id"];
		            update("E[leaving](check id=" + param.value + ", hasChanged=" + param.hasChanged + ")");
		        });

		        Path.map("#F").before(function() {
		            update("F[before]");
		        }).to(function() {
		            update("F[action]");
		        });

		        Path.map("#G").before(function() {
		            update("G[before 1]");
		        }).before(function() {
		            update("G[before 2]");
		        }).before([
